...
func (a *RecommendationAnalyzer) analyzeMetric(
	...
	if utilization.LessThan(veryLowUtilizationThreshold) && !peak.IsZero() {
		...
		return &models.CostRecommendation{
			...
			Description:        fmt.Sprintf("The %s metric shows very low utilization (%s%%). Peak usage is %s but average is only %s. Consider downsizing to reduce costs.", metric, utilization.StringFixed(1), peak.StringFixed(2), average.StringFixed(2)),
			...
			RecommendedAction:  fmt.Sprintf("Reduce capacity to match average usage of %s %s", average.StringFixed(2), usages[0].Unit),
		...
		return &models.CostRecommendation{
			...
			Description:        fmt.Sprintf("The %s metric shows low utilization (%s%%). Peak usage is %s but average is only %s. Consider rightsizing to optimize costs.", metric, utilization.StringFixed(1), peak.StringFixed(2), average.StringFixed(2)),
			CurrentCost:        totalCost,
			PotentialSavings:   potentialSavings,
			Currency:           "USD",
			Metric:             metric,
			...
			AverageValue:       average,
			UtilizationPercent: utilization,
			RecommendedAction:  fmt.Sprintf("Adjust capacity to better match average usage of %s %s", average.StringFixed(2), usages[0].Unit),
			AnalysisPeriod:     fmt.Sprintf("%d days", int(endDate.Sub(startDate).Hours()/24)),
			StartDate:          startDate,
			EndDate:            endDate,
			CreatedAt:          time.Now(),
